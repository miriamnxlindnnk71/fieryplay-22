...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-aabdc566";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-aabdc566">'+'<span class="tile-face-aabdc566 tile-front-aabdc566"></span>'+'<span class="tile-face-aabdc566 tile-back-aabdc566"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-aabdc566")){return;}
var back=tile.querySelector(".tile-back-aabdc566");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-aabdc566");if(isMine){tile.classList.add("boom-aabdc566");}
if(dim){tile.classList.add("dimmed-aabdc566");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-aabdc566")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-aabdc566");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-aabdc566")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-aabdc566");void panelEl.offsetWidth;panelEl.classList.add("jolt-aabdc566");}
endRound();}
